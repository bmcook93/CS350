...
class TemperatureMachine(StateMachine):
    ...
    def updateLights(self):
        ...
        if state == "heat":
            if temp < self.setPoint: # If below setpoint, pulse red
                redLight.pulse()
            else: # Remain solid when at setpoint
                redLight.value = 1
        elif state == "cool":
            if temp > self.setPoint: # If above setpoint, pulse blue
                blueLight.pulse()
            else:
                blueLight.value = 1
        else: # Off
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            lcd_line_1 = current_time.strftime("%Y-%m-%d %H:%M:%S").ljust(16)[:16]
    
            ## Setup Display Line 2
            if(altCounter < 6):
                current_temp = floor(self.getFahrenheit())
                lcd_line_2 = f"Temp:{current_temp}F".ljust(16)[:16]
    
                altCounter = altCounter + 1
            else:
                lcd_line_2 = f"Mode:{self.current_state.id} T:{self.setPoint}F".ljust(16)[:16]
